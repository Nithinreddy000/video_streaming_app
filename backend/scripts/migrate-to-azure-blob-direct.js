/**
 * Migration Script: Update all video URLs from Fastly CDN to Azure Blob Storage Direct
 *
 * This script updates all existing video documents in MongoDB to use
 * Azure Blob Storage URLs directly instead of Fastly CDN URLs.
 *
 * Usage: node scripts/migrate-to-azure-blob-direct.js
 */
...
const OLD_CDN_HOSTNAME = 'videosentinel.global.ssl.fastly.net';
const NEW_CDN_HOSTNAME = 'videosentinel.blob.core.windows.net/videosentinel-uploads';

/**
 * Replace CDN hostname in a URL
 */
function replaceAzureBlobUrl(url) {
 ...
 */
async function migrateToAzureBlobDirect() {
  try {
    console.log('🚀 Starting migration to Azure Blob Storage (Direct Access)...\n');
    ...
    // Find all videos with Fastly URLs
    const videosWithFastly = await Video.find({
      $or: [
    ...
    console.log(`📊 Found ${videosWithFastly.length} videos with Fastly URLs\n`);

    if (videosWithFastly.length === 0) {
      console.log('✅ No videos to migrate. All videos already using Azure Blob Storage.');
    ...
    // Update each video
    for (const video of videosWithFastly) {
      try {
        ...
        if (video.cdnUrl && video.cdnUrl.includes(OLD_CDN_HOSTNAME)) {
          updates.cdnUrl = replaceAzureBlobUrl(video.cdnUrl);
          hasChanges = true;
        ...
        if (video.thumbnailUrl && video.thumbnailUrl.includes(OLD_CDN_HOSTNAME)) {
          updates.thumbnailUrl = replaceAzureBlobUrl(video.thumbnailUrl);
          hasChanges = true;
        ...
        if (video.processedFiles) {
          ...
          if (processedFiles.hls && processedFiles.hls.includes(OLD_CDN_HOSTNAME)) {
            processedFiles.hls = replaceAzureBlobUrl(processedFiles.hls);
          ...
          if (processedFiles.dash && processedFiles.dash.includes(OLD_CDN_HOSTNAME)) {
            processedFiles.dash = replaceAzureBlobUrl(processedFiles.dash);
            processedFilesChanged = true;
          ...
          if (processedFiles.thumbnail && processedFiles.thumbnail.includes(OLD_CDN_HOSTNAME)) {
            processedFiles.thumbnail = replaceAzureBlobUrl(processedFiles.thumbnail);
            processedFilesChanged = true;
          ...
          if (processedFiles.poster && processedFiles.poster.includes(OLD_CDN_HOSTNAME)) {
            processedFiles.poster = replaceAzureBlobUrl(processedFiles.poster);
            processedFilesChanged = true;
          ...
          if (processedFiles.mp4 && processedFiles.mp4 instanceof Map) {
            ...
            for (const [quality, url] of processedFiles.mp4) {
              if (url && url.includes(OLD_CDN_HOSTNAME)) {
                newMp4Map.set(quality, replaceAzureBlobUrl(url));
                mp4Changed = true;
              } else {
    ...
    console.log(`📁 Total processed: ${videosWithFastly.length} videos`);
    console.log('='.repeat(60));
    console.log('\n✅ Migration completed!\n');

    // Verify the migration
    console.log('🔍 Verifying migration...');
    const remainingFastly = await Video.countDocuments({
      $or: [
        { cdnUrl: { $regex: OLD_CDN_HOSTNAME } },
        { 'processedFiles.hls': { $regex: OLD_CDN_HOSTNAME } }
      ]
    ...
    if (remainingFastly === 0) {
      console.log('✅ Verification passed: All videos now using Azure Blob Storage\n');
    } else {
      console.log(`⚠️  Warning: ${remainingFastly} videos still have Fastly URLs\n`);
    }
