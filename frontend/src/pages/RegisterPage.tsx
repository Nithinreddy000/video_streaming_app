...
import { UserPlus, AlertCircle, Mail, Lock, Eye, EyeOff, ArrowRight, User, Building2, CheckCircle } from 'lucide-react';
...
export const RegisterPage = () => {
  ...
  if (formData.password) {
    ...
    if (score <= 2) {
      ...
      passwordStrength.color = 'from-red-500 to-red-600';
    } else if (score <= 3) {
      passwordStrength.score = 2;
      passwordStrength.label = 'Fair';
      passwordStrength.color = 'from-yellow-500 to-yellow-600';
    } else if (score <= 4) {
      passwordStrength.score = 3;
      passwordStrength.label = 'Good';
      passwordStrength.color = 'from-blue-500 to-blue-600';
    } else {
      passwordStrength.score = 4;
      passwordStrength.label = 'Strong';
      passwordStrength.color = 'from-green-500 to-green-600';
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAuth(
        response.user,
        response.tenant || ({} as any),
        {
          accessToken: response.tokens.accessToken,
          refreshToken: response.tokens.refreshToken,
          expiresIn: response.tokens.expiresIn || '15m',
        }
      );
      ...
      >
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            >
              Create your account to get started with AI-powered video processing
            </motion.p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <motion.div variants={itemVariants} className="space-y-2">
                ...
                <div className="relative">
                  <User className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-slate-500 pointer-events-none" />
                  <input
                    id="username"
                    name="username"
                    type="text"
              ...
              {/* First Name field */}
              <motion.div variants={itemVariants} className="space-y-2">
                <label htmlFor="firstName" className="text-sm font-medium text-slate-300 block">
                  First Name
                </label>
                <input
                  id="firstName"
                  ...
                  onChange={handleChange}
                  disabled={isLoading}
                  className="w-full bg-slate-700/50 border border-slate-600 rounded-lg px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all disabled:opacity-50"
                />
              </motion.div>

              {/* Last Name field */}
              <motion.div variants={itemVariants} className="space-y-2">
                <label htmlFor="lastName" className="text-sm font-medium text-slate-300 block">
                  Last Name
                </label>
                <input
                  id="lastName"
              ...
              <motion.div variants={itemVariants} className="md:col-span-2 space-y-2">
                <label htmlFor="organizationName" className="text-sm font-medium text-slate-300 block">
                  Organization Name
                </label>
                <div className="relative">
                  <Building2 className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-slate-500 pointer-events-none" />
                  <input
                    id="organizationName"
